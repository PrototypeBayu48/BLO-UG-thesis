...
def quadmpc(xinitial,xfinal,xnext,u0,cpoint_tol,maxiter,trajgen = 0,geffect = 0):
    t1 = time()

    itertime = np.zeros(maxiter + 1)
...
def quadmpc_koop(xinitial,xfinal,xnext,u0,cpoint_tol,maxiter,n,A_array,B_array,C_array,basis,trajgen = 0):
    ...
    # upper bounds of the input
    mpc.bounds['upper','_u','w1'] = 10.0
    mpc.bounds['upper','_u','w2'] = 10.0
    mpc.bounds['upper','_u','w3'] = 10.0
    mpc.bounds['upper','_u','w4'] = 10.0
    ...
    print('Koopman MPC time = ')
    print(time() - t1)

    return [t_f,x_f,u_f,itertime]

def quadmpc_koop_realsim(xinitial,xfinal,xnext,u0,cpoint_tol,maxiter,n,A_array,B_array,C_array,basis,trajgen = 0):
    t1 = time()
    ...
    model_type_koop = 'discrete' # either 'discrete' or 'continuous'
    model_koop = do_mpc.model.Model(model_type_koop)

    z = model_koop.set_variable(var_type='_x', var_name='z', shape=(n,1))
    w1 = model_koop.set_variable(var_type='_u', var_name='w1')
    w2 = model_koop.set_variable(var_type='_u', var_name='w2')
    w3 = model_koop.set_variable(var_type='_u', var_name='w3')
    w4 = model_koop.set_variable(var_type='_u', var_name='w4')
    ...
    mterm = 100*(z[1] - xf)**2 + 100*(z[2] - yf)**2 + 100*(z[3] - zf)**2 + (z[7] - dx_t)**2 + (z[8] - dy_t)**2 + (z[9] - dz_t)**2
    lterm = 100*(z[1] - xf)**2 + 100*(z[2] - yf)**2 + 100*(z[3] - zf)**2 + (z[7] - dx_t)**2 + (z[8] - dy_t)**2 + (z[9] - dz_t)**2
...
def quadsim_koop(xset,uset,maxiter,n,A_array,B_array,C_array,basis):
    model_type = 'discrete' # either 'discrete' or 'continuous'
    model = do_mpc.model.Model(model_type)
...
def trajgen(tspan,xspan,yspan,zspan,ustart,cpoint_tol_trajgen,maxiter_trajgen,A_array,B_array,C_array,basis,koop = 0,realsim = 0,trajgen_geffect = 0):
    trajgen_iter = 1
    ...
    if koop == 1:
        if realsim == 1:
            [tfinal,xfinal,ufinal,itertime_total] = quadmpc_koop_realsim(x1,x2,x3,ustart,cpoint_tol_trajgen,maxiter_trajgen,23,A_array,B_array,C_array,basis)
        else:
            [tfinal,xfinal,ufinal,itertime_total] = quadmpc_koop(x1,x2,x3,ustart,cpoint_tol_trajgen,maxiter_trajgen,23,A_array,B_array,C_array,basis)
    else:
        [tfinal,xfinal,ufinal,itertime_total] = quadmpc(x1,x2,x3,ustart,cpoint_tol_trajgen,maxiter_trajgen,geffect = trajgen_geffect)
    ...
    for n1 in range(2,len(tspan)):
        ...
        if koop == 1:
            if realsim == 1:
                [tstep,xstep,ustep,itertime_step] = quadmpc_koop_realsim(x1,x2,x3,ustep,cpoint_tol_trajgen,maxiter_trajgen,23,A_array,B_array,C_array,basis)
            else:
                [tstep,xstep,ustep,itertime_step] = quadmpc_koop(x1,x2,x3,ustep,cpoint_tol_trajgen,maxiter_trajgen,23,A_array,B_array,C_array,basis)
        else:
            [tstep,xstep,ustep,itertime_step] = quadmpc(x1,x2,x3,ustep,cpoint_tol_trajgen,maxiter_trajgen,geffect = trajgen_geffect)
        ...
        itertime_total = np.concatenate((itertime_total,itertime_step))
        ustep = np.array([[ustep[len(tstep)-1,0]], [ustep[len(tstep)-1,1]], [ustep[len(tstep)-1,2]], [ustep[len(tstep)-1,3]]])
        t_targ = np.concatenate((t_targ,[tfinal[len(tfinal)-1]]))
    
    return [tfinal,t_targ,xfinal,ufinal,itertime_total]
...
for n1 in range(n_traj_train):
    ...
    for i in range(len(t_dataset_train)):
        x_dataset_train[i] = -0.5 + rand.random()*1.0
        y_dataset_train[i] = -0.5 + rand.random()*1.0
        z_dataset_train[i] = 0.02 + rand.random()*0.18
    [t_iter,t_iter_targ,x_iter,u_iter,trainitertime] = trajgen(t_dataset_train,x_dataset_train,y_dataset_train,z_dataset_train,uhover,0.0,100,1,1,1,1,trajgen_geffect = 1)
...
x_train_new = np.load('x_train_7.npy')
u_train_new = np.load('u_train_7.npy')
x_val_new = np.load('x_val_7.npy')
u_val_new = np.load('u_val_7.npy')

x_train_new[0:2,:,:] = x_train
u_train_new[0:2,:,:] = u_train

np.save('x_train_10',x_train_new)
np.save('u_train_10',u_train_new)
np.save('x_val_10',x_val_new)
np.save('u_val_10',u_val_new)

plt.figure()
plt.plot(t_train,x_train[n_traj_train - 1,:,0],label = 'x val')
plt.plot(t_train,x_train[n_traj_train - 1,:,1],label = 'y val')
plt.plot(t_train,x_train[n_traj_train - 1,:,2],label = 'z val')
plt.plot(t_dataset_train,x_dataset_train,'o',label = 'x val target')
plt.plot(t_dataset_train,y_dataset_train,'o',label = 'y val target')
plt.plot(t_dataset_train,z_dataset_train,'o',label = 'z val target')
plt.legend()
plt.show()

plt.figure()
ax = plt.axes(projection='3d')
ax.plot3D(x_train[n_traj_train - 1,:,0],x_train[n_traj_train - 1,:,1],x_train[n_traj_train - 1,:,2],label='val trajectory')
ax.plot3D(x_dataset_train,y_dataset_train,z_dataset_train,'o',label='val target trajectory')
plt.legend()
plt.show()
