...
for n1 in range(n_traj_train):
    ...
    for i in range(len(t_dataset_train)):
        ...
        z_dataset_train[i] = 0.05 + rand.random()*0.95
...
for n2 in range(n_traj_val):
    ...
    for i in range(len(t_dataset_val)):
        x_dataset_val[i] = -0.5 + rand.random()*1.0
        y_dataset_val[i] = -0.5 + rand.random()*1.0
        z_dataset_val[i] = 0.05 + rand.random()*0.95
    [t_iter,t_iter_targ,x_iter,u_iter,valitertime] = trajgen(t_dataset_val,x_dataset_val,y_dataset_val,z_dataset_val,uhover,0.0,100,1,1,1,1,trajgen_geffect = 1)
    u_iter = u_iter[0:len(t_iter)-1,:]
    x_val[n2,:,:] = x_iter
    u_val[n2,:,:] = u_iter
    t_val = t_iter
...
np.save('x_train_7',x_train)
np.save('u_train_7',u_train)
np.save('x_val_7',x_val)
np.save('u_val_7',u_val)
